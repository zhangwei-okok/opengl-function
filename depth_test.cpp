...
void init() {
    ...
    if (!shader->build("../shader/texture.vert", "../shader/deepFrag.glsl")) {
...
void render() {
    ...
    auto mvp = vmath::ortho(0, SCR_WIDTH, SCR_HEIGHT, 0, -10, 300);
    auto view = vmath::lookat(vmath::vec3(SCR_WIDTH / 2, SCR_HEIGHT / 2, 500),
                              vmath::vec3(SCR_WIDTH / 2, SCR_HEIGHT / 2, 0), vmath::vec3(0, 1, 0));
    auto pro = vmath::perspective(45.0f, (float) SCR_WIDTH / (float) SCR_HEIGHT, 1, 3400);
    mvp = pro * view;
    ...
    saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "depth_test");
