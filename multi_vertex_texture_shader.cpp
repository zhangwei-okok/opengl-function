...
unsigned int *vao, *vbo, textureID1, textureID2;
unsigned int *texVBO;
...
void init() {
    ...
    textureID1 = gentexture("../static/55.jpg");
    textureID2 = gentexture("../static/lighting_maps_specular_color.png");
...
void render() {
    for (int k = 0; k < 3; k++) {
        ...
        for (int i = 0; i < points.size() / 9; ++i) {
            ...
            // std::cout << x << " - " << y << "\n";
            view *= vmath::translate<float>(x, y, 0);
            view *= vmath::rotate<float>(glfwGetTime() * 200, 0, 0, 1);
            view *= vmath::translate<float>(-x, -y, 0);
            mvp = mvp * view;
            shader->setUniformMatrix4fv("MVP", mvp.operator float *());
        ...
        for (int i = 0; i < points.size() / 9; ++i) {
            ...
            // std::cout << x << " - " << y << "\n";
            view *= vmath::translate<float>(x, y, 0);
            view *= vmath::rotate<float>(glfwGetTime() * 50, 0, 0, 1);
            view *= vmath::translate<float>(-x, -y, 0);
            mvp = mvp * view;
            shader->setUniformMatrix4fv("MVP", mvp.operator float *());
        ...
        switch (k) {
            case 0:
                saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "multi_vertex_texture_shader_1");
                break;
            case 1:
                saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "multi_vertex_texture_shader_2");
                break;
            case 2:
                saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "multi_vertex_texture_shader_3");
                break;
        }
...
//        if (cnt > 1500) {
//            glReadPixels(0, 0, SCR_WIDTH, SCR_HEIGHT, GL_BGRA, GL_UNSIGNED_BYTE, &bmpData[0]);
//            saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "transform_trangle_texture_color_transform");
//            return;
//        }
