...
void render() {
    ...
    for (int i = 0; i < points.size() / 9; ++i) {
        ...
        view *= vmath::rotate<float>(glfwGetTime() * 50 + 50, 0, 0, 1);
        view *= vmath::translate<float>(-x, -y, 0);
        mvp = mvp * view;
    ...
    for (int i = 0; i < points.size() / 9; ++i) {
        ...
        // std::cout << x << " - " << y << "\n";
        view *= vmath::translate<float>(x, y, 0);
        view *= vmath::rotate<float>(glfwGetTime() * 20, 0, 0, 1);
        view *= vmath::translate<float>(-x + 1, -y + 1, 0);
        mvp = mvp * view;
        shader->setUniformMatrix4fv("MVP", mvp.operator float *());
    ...
    for (int i = 0; i < points.size() / 9; ++i) {
        ...
        // std::cout << x << " - " << y << "\n";
        view *= vmath::translate<float>(x, y, 0);
        view *= vmath::rotate<float>(glfwGetTime() * 10, 0, 0, 1);
        view *= vmath::translate<float>(-x, -y, 0);
        mvp = mvp * view;
        shader->setUniformMatrix4fv("MVP", mvp.operator float *());
    ...
    saveBmpForRGBA(&bmpData[0], SCR_WIDTH, SCR_HEIGHT, "single_vertex_texture_shader");
